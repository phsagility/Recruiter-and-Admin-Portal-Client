...
function renderHistoryTrendGraph() {
  ...
  container.innerHTML = chartTypes.map(({ title, field }) => {
    ...
    return `<div class="history-trend-chart"><div class="history-trend-period">${monthLabel}</div><h3>${title}</h3><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${title} for ${selectedMonth}">${grid}${lines}${labels}</svg></div>`;
  }).join('');
}
